"""This module provides the StopRule base class.

It is the base class to be used for custom subclasses involving stop-loss exits.
"""
...
class StopRule(ABC):
    """This class provides a base class for stop-loss exits.
    """ 
    def __init__(self, entry_price: npt.NDArray, trade_type: npt.NDArray) -> None:
        """Initialises the StopRule class.
    ...
    def hit(self, bar: pd.Series) -> bool:
        """Checks if the current bar triggers a stop-loss exit.
        ...
        -------
        bool
            True if the current price is below the stop-loss price level, False otherwise
        """
        pass

    @abstractmethod
    def exit_mask(self, df: pd.DataFrame) -> npt.NDArray:
        """Creates a boolean mask checking which prices are below the stop-loss price level.
        ...
        -------
        npt.NDArray
            The boolean array specifying which prices are below stop-loss price levels
        """ 
        pass

    @abstractmethod
    def update(self, bar: pd.Series) -> None:
        """Updates the stop-loss price level, based on current bar.
